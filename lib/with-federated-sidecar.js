...
class NextFederationPlugin {
    constructor(federationOptions, nextOptions) {
        this.federationOptions = federationOptions;
        this.nextOptions = nextOptions;
    }

    apply(compiler) {
        compiler.hooks.make.tapAsync('next-federation', (compilation,callback) => {
            console.log(this.federationOptions);
            console.log(compiler.options.output);
            const childCompiler = compilation.createChildCompiler('next-federation-child', {...compiler.options.output}, [
                new this.nextOptions.webpack.container.ModuleFederationPlugin({
                    ...this.federationOptions,
                    runtime: false
                }),
            ]);
            childCompiler.context = compilation.options.context
            // this.nextOptions.webpack.EntryOptionPlugin.applyEntryOption(childCompiler, childCompiler.context, {
            //     child: {
            //         import: path.join(__dirname,'./noop.js'),
            //     },
            // });
            ...
            return Object.assign({}, nextConfig, {
                ...
                webpack(config, options) {
                    ...
                    if (!options.isServer) {
                        ...
                        config.output.uniqueName= federationPluginOptions.name,
                            config.cache= false
                        config.plugins.push(
                            new NextFederationPlugin(federationPluginOptions, options),
                            new webpack.container.ModuleFederationPlugin({...federationPluginOptions, filename: 'noop-remote.js'}),
                        )
                        /**
                        ...
                        //    */
                        //   apply(compiler) {
                        //     console.log(options.config.configFile)
                        //     // console.log(baseConfig)
                        //     // console.log(defaultConfig)
                        //     const combinedConfig = Object.assign(
                        //         defaultConfig,
                        //         require(options.config.configFile),
                        //     )
                        //
                        //     const finalConfig = Object.assign(combinedConfig,{
                        //       distDir: path.join(combinedConfig.distDir,'ssr'),
                        //       buildId: 'federation'
                        //     })
                        //
                        //     console.log(finalConfig)
                        //
                        //     const fedConfig = options.config.configFile.replace('next.config','federation.config')
                        //     fs.writeFileSync(fedConfig,'module.exports = {}','utf-8');
                        //     require(fedConfig)
                        //
                        //     console.log(require.cache[fedConfig].exports);
                        //     require.cache[fedConfig].exports = require(options.config.configFile);
                        //     // console.log('config',require(fedConfig));
                        //     console.log('building next webpack');
                        //     // console.log(combinedConfig)
                        //     console.log('done building next webpack');
                        //     const run = (compilation, done) => {
                        //       const build=require('next/dist/build').default
                        //       build(path.dirname(options.config.configFile),finalConfig)
                        //       return
