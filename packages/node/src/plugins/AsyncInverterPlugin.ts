        ...
        (source, renderContext) => {
          ...
          if (replaceSource[0].includes(searchString)) {
            ...
            replaceSource[1] = `
              ...
              ${initialShareMaps}
// const scope = __webpack_require__.S ? __webpack_require__.I('default') : null

            var ${searchString} = function(moduleId) {
            var chunkRequirements = Object.values(listOfInitialIds).reduce((acc, val) => acc.concat(val), [])
            // return __webpack_require__.X(0, [...(chunkRequirements||[])], () => {
            // console.log('after load');
            // // return __webpack_require__.I('default');
            // }).then(()=>{
            //         console.log(__webpack_require__.m[moduleId]);
            // return __webpack_exec_proxy__(moduleId);
            // })
            // console.log('not resolving', moduleId);
