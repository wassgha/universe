...
export default function patchDefaultSharedLoader(
  ...
  if (
    content.includes('hasShareHoist')
  ) {
    return content;
  }
  let shareScopes='';
  ...
  return ['console.log(\'initializing internal-module-hoist for:\', __webpack_runtime_id__)',shareScopes,eagerShared.sideload, '//hasShareHoist', content].join('\n');
}
